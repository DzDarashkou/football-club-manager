...
const { data, refresh, pending, error: loadError } = await useFetch<AdminTrainingsResponse>('/api/admin/trainings', { default: () => ({ trainings: [] }) })
...
function format(value: string) { return new Intl.DateTimeFormat('pl-PL', { dateStyle: 'medium', timeStyle: 'short', timeZone: 'Europe/Warsaw' }).format(new Date(value)) }
  ...
  <div class="mx-auto max-w-7xl space-y-6">
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <Card class="overflow-hidden p-0">
        <div class="border-b border-border p-4"><h2>Treningi</h2></div>
        <p v-if="pending" class="p-4 text-sm">Wczytywanie treningów...</p>
        <p v-else-if="loadError" role="alert" class="p-4 text-sm">Nie udało się wczytać treningów. <Button variant="outline" @click="refresh()">Spróbuj ponownie</Button></p>
        <p v-else-if="!data?.trainings.length" class="p-6 text-sm">Nie zaplanowano jeszcze treningów.</p>
        <div v-for="training in data?.trainings" :key="training.id" class="space-y-3 border-b border-border p-4 last:border-0">
          <div class="flex flex-wrap items-center gap-3"><Dumbbell class="h-5 w-5 shrink-0 text-brand-700" /><NuxtLink :to="`/coach/calendar/trainings/${training.id}`" class="min-h-11 py-2 font-medium text-brand-700">{{ training.team.name }}</NuxtLink><TrainingsStatusBadge :status="training.status" /></div>
          <p class="text-sm">{{ format(training.scheduled_at) }} · {{ training.venue?.name || 'Miejsce do potwierdzenia' }}</p>
          <p v-if="training.original_scheduled_at" class="text-sm">Pierwotny termin: {{ format(training.original_scheduled_at) }}</p>
          <TrainingsSessionEditor :training="training" @saved="refresh()" />
        </div>
      </Card>
