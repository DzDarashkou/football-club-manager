...
import { CalendarPlus, MapPin, Pencil, Plus, Search, Trash2, Trophy } from 'lucide-vue-next'
...
const venue = reactive({ name: '', address: '', city: '', latitude: '', longitude: '' })
...
const game = reactive<GameForm>({ team_id: '', season_id: '', competition_id: null, venue_id: null, opponent_name: '', location_type: 'home', scheduled_at: '', matchday: '', round_label: '', status: 'scheduled', has_broadcast: false, broadcast_url: null, home_score: 0, away_score: 0, notes: '' })
...
async function createVenue() { await create('/api/admin/venues', { ...venue, name: venue.name.trim(), address: venue.address.trim() || null, city: venue.city.trim() || null, latitude: venue.latitude === '' ? null : Number(venue.latitude), longitude: venue.longitude === '' ? null : Number(venue.longitude) }, 'Miejsce zostało utworzone.'); venue.name = ''; venue.address = ''; venue.city = ''; venue.latitude = ''; venue.longitude = '' }
async function createGame() { saving.value = true; try { await $fetch('/api/admin/games', { method: 'POST', body: { ...game, opponent_name: game.opponent_name.trim(), scheduled_at: new Date(game.scheduled_at).toISOString(), matchday: game.matchday === '' ? null : game.matchday, round_label: game.round_label.trim() || null, notes: game.notes.trim() || null } }); await refreshGames(); message('Mecz został utworzony.'); isCreatePanelOpen.value = false; game.opponent_name = ''; game.scheduled_at = ''; game.matchday = ''; game.round_label = ''; game.home_score = 0; game.away_score = 0; game.notes = ''; game.has_broadcast = false; game.broadcast_url = null } catch (value) { problem(value, 'Nie udało się utworzyć meczu.') } finally { saving.value = false } }
