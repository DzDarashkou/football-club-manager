...
<template>
  <div class="mx-auto max-w-4xl space-y-6">
    ...
    <template v-else>
      <div class="space-y-2"><p class="eyebrow text-brand-700">Dzień meczu</p><h1>{{ game.location_type === 'away' ? game.opponent_name + ' – ' + game.team.name : game.team.name + ' – ' + game.opponent_name }}</h1><p class="flex items-center gap-2 text-body text-[color:var(--color-text-secondary)]"><CalendarClock class="h-4 w-4" />{{ formatDate(game.scheduled_at) }}</p></div>
      ...
      <AppBroadcastLink :game="game" /><AppMatchWeatherCard v-if="weather?.status === 'available' && data" :weather="weather" :attribution="data.weatherAttribution" />
