...
import { CalendarClock, Check, ExternalLink, MapPin, X } from 'lucide-vue-next'
...
<template>
  <div class="mx-auto max-w-4xl space-y-6">
    <NuxtLink to="/coach/calendar" class="text-sm text-brand-700">← Calendar</NuxtLink>
    ...
    <template v-else>
      ...
      <Card class="overflow-hidden p-0"><div class="border-b border-border p-4"><h2>Requested players</h2><p class="mt-1 text-sm text-[color:var(--color-text-secondary)]">Confirm or reject availability after speaking with a parent.</p></div><p v-if="!players.length" class="p-6 text-center text-sm text-[color:var(--color-text-secondary)]">No players have been requested for this game yet.</p><div v-for="player in players" :key="player.player_id" class="flex flex-col gap-3 border-b border-border p-4 last:border-b-0 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-medium">{{ player.player.full_name }}</p><Badge class="mt-1" :status="attendanceBadge(player.availability_status)">{{ attendanceLabel(player.availability_status) }}</Badge></div><div class="grid grid-cols-2 gap-2 sm:flex"><Button size="sm" class="min-h-11 gap-1.5" :disabled="savingPlayerId !== null || player.availability_status === 'available'" @click="setAttendance(player, 'available')"><Check class="h-4 w-4" />Confirm</Button><Button variant="outline" size="sm" class="min-h-11 gap-1.5 border-[var(--status-declined-ring)] text-[var(--status-declined-text)] hover:bg-[var(--status-declined-bg)]" :disabled="savingPlayerId !== null || player.availability_status === 'unavailable'" @click="setAttendance(player, 'unavailable')"><X class="h-4 w-4" />Reject</Button></div></div></Card>
    </template>
  </div>
</template>
